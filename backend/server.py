...
EMAIL_FROM_NAME = os.environ.get("EMAIL_FROM_NAME", "FieldForge")
...
async def send_estimate(item_id: str, user: dict = Depends(require_role("owner", "foreman"))):
    ...
    <table width='100%' style='font-family:Arial,sans-serif;max-width:600px;margin:auto'>
      <tr><td><h2 style='color:#FF3B30'>{comp.get('name','FieldForge')} — Estimate</h2>
    ...
    payload = {"to": [est["customer_email"]], "subject": f"Your estimate from {comp.get('name','FieldForge')}", "html": html, "from_name": EMAIL_FROM_NAME}
    try:
